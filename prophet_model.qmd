...
```{r plot, echo=FALSE, warning=FALSE, message=FALSE}
  ...
  forecast_data <- pins::pin_get('forecast_cxn_auto', board = 'rsconnect')$ltoForecast %>% filter(day>=Sys.Date()-725)
  actuals_data <- pins::pin_get('forecast_cxn_auto', board = 'rsconnect')$ltoActual %>% filter(day>=Sys.Date()-725)
  my_product = 'rto'
  time = 'day'

  # forecast_data <- forecastData$ltoForecast
  # actuals_data <- forecastData$ltoActual

  if(my_product == 'rto') {
    my_title = 'LTO Collections: Forecast vs. Actuals'
  } else {
    my_title = 'SLN Collections: Forecast vs. Actuals'
  }
  ...
  if(time=='day'){
    plot_data <- forecast_data %>% 
      rename(x=day) %>% 
      merge(
        actuals_data %>% filter(day < Sys.Date()) %>% rename(x=day),
        by.x = 'x',
        all.x = TRUE
      ) %>% 
      mutate(
    ...
    plot_data <- forecast_data %>% 
      mutate(
        x = floor_date(day,'week',week_start = 1)
      ) %>% 
      group_by(x) %>% 
      summarise(forecast=sum(forecast),
                low_forecast=sum(low_forecast),
                high_forecast=sum(high_forecast)) %>% 
      merge(
        actuals_data %>% 
          filter(day < Sys.Date()) %>% 
          mutate(x=floor_date(day,'week',week_start = 1)) %>% 
          group_by(x) %>% 
          summarise(actual=round(sum(actual),0))
          ,
        by.x = 'x',
        all.x = TRUE
      ) %>% 
